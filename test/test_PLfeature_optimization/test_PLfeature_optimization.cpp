...
//        cv::imshow(" image line show ", imagelineshow);
    ...
    pKeyFrame = new PL_VO::KeyFrame(*plastFrame, pMap);
    pKeyFrame->mpFrame = plastFrame;
    plastFrame->mpKeyFrame = pKeyFrame;

    pTracking->SetCurLastFrame(pcurrentFrame, plastFrame);

    pTracking->SetLastKeyFrame(pKeyFrame);

    pTracking->TrackRefFrameByPnP(vpointRefineMatches, vlineRefineMatches);
...
//    pcurrentFrame->UnprojectStereo(img2depth, vpointRefineMatches, vlineRefineMatches);
    ...
    PL_VO::Optimizer::PoseOptimization(pcurrentFrame, pKeyFrame);
    ...
    cv::line_descriptor::drawLineMatches(img1, plastFrame->mvKeyLineUn, img2, pcurrentFrame->mvKeyLineUn,
                                         vlineRefineMatches, showimg2,  cv::Scalar::all(-1), cv::Scalar::all(-1), mask,
                                         cv::line_descriptor::DrawLinesMatchesFlags::DEFAULT);
    cv::imshow("point ", showimg);
    cv::imshow("line ", showimg2);
